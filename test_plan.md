...
- `✅` means file is covered by unit tests.
- Coverage source:
  - Specialized unit tests in `tests/unit/...`
  - Global per-file AST unit test: `tests/unit/test_meta/test_all_source_files_ast.py`
...
- Existing focused unit tests remain in `tests/unit/...`.
- Global per-file AST coverage test guarantees every source file has at least one unit test assertion path.

...
class Image2Image(BaseTool, BaseTaskWidget):

    def __init__(self, workspace, editor=None):
        super(Image2Image, self).__init__(workspace)
        self.setObjectName("tool_image_to_image")
    ...
    def init_ui(self, main_editor):
        """Initialize the UI for the image2image tool"""
    ...
    def get_media_path(self, timeline_item):
        """Get media path for image2image tool"""
        return timeline_item.get_image_path()

    @asyncSlot()
    async def execute(self, task):
        # Only process image2image tasks
        if task.tool != "image2image":
            return  # Exit early if this is not an image2image task
        try:
            import logging
            logger = logging.getLogger(__name__)
            logger.info(f"Processing image2image task with FilmetoApi: {task.options}")
            ...
            logger.info(f"Image2Image: width={width}, height={height}, prompt={prompt[:50]}...")
            ...
            logger.info(f"Image2Image: using ability selection (auto mode)")
            ...
            async for update in api.execute_task_stream(filmeto_task):
                if isinstance(update, FilmetoTaskProgress):
                    ...
                    logger.info(f"Image2Image completed: status={update.status}, server={filmeto_task.server_name}, model={filmeto_task.model_name}")
            ...
            logger.error(f"Error in Image2Image.execute: {e}", exc_info=True)
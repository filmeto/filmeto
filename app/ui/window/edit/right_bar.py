...
class RightBar:
    ...
    def __init__(
        ...
        if defer_sidebar:
            self._shell = QFrame()
            self._shell.setObjectName("edit_right_tool_strip_shell")
            ...
            self._shell.setStyleSheet(
                "QFrame#edit_right_tool_strip_shell {"
    ...
    def attach_sidebar(self) -> None:
        """Swap tool-strip skeleton for MainWindowRightSideBar (main thread)."""
        if self.bar is not None or self._shell is None:
            return
        self.bar = MainWindowRightSideBar(self.workspace, self.parent)

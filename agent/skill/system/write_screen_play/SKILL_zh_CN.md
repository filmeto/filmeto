...
description: |-
  用途：创建、开发和管理符合好莱坞标准的长篇电影或网络连续剧剧本。
  能力：多模式操作（创作/指令/分析/优化）、场景增删改查、角色管理、故事结构指导。
  触发：当用户想要"写剧本"、"创建场景"、"修改场景"、"分析剧本"或任何剧本相关的创作工作时。
  排除：不用于任何删除操作。删除场景请使用 delete_single_scene 或 delete_screen_play 技能。
tools:
  - screen_play
---
...
| 修改现有内容（除删除外） | **指令模式** | "改"、"修改"、"更新"、"重命名"、"修复" |
| 查询/分析内容 | **分析模式** | "显示"、"列出"、"阅读"、"分析"、"哪些场景"、"哪些角色" |
| 优化剧本结构 | **优化模式** | "优化"、"重组"、"整理"、"改进结构"、"修复编号"、"润色" |

**重要**：删除操作使用专门的技能：
- **删除单个场景** → 使用 `delete_single_scene` 技能
- **删除整个剧本** → 使用 `delete_screen_play` 技能
